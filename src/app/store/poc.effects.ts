...
const hostlink = 'http://4.174.172.228'; // Replace with your own REST API server URL
...
export class pocEffects {
    getServerStatus$ = createEffect(() => this._actions$.pipe(
        ...
        switchMap(() => {
            return this._http.get<string>(hostlink).pipe(
                map((f: any) => PacActions.getServerStatusSuccess({status: f})),
